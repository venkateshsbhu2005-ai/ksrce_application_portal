...
const ManageMentors = () => {
  ...
  const onSingleSubmit = async (data) => {
    ...
    try {
      ...
      setSingleMessage({ type: 'error', text: err.response?.data?.message || 'Failed to add mentor.' });
    } finally {
  ...
  const onBulkSubmit = async () => {
    ...
    try {
      const res = await api.post('/admin/mentors/upload', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      setBulkMessage({ type: 'success', text: res.data.message });
      setFile(null);
    } catch (err) {
      setBulkMessage({ type: 'error', text: err.response?.data?.message || 'Failed to upload mentors.' });
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    setEditLoading(true);
    try {
      await api.put(`/admin/user/${editingMentor.id}`, editingMentor);
  ...
  const downloadTemplate = () => {
    const csvContent = "Name,Email,Department\nJohn Mentor,mentor.john@college.edu,Computer Science";
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">Manage Mentors</h2>
        <p className="text-gray-500">View, edit, or add mentors to the system.</p>
      </div>
      ...
      {activeTab === 'list' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>A list of all mentors currently in the system.</CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {mentors.length === 0 ? (
                      <tr><td colSpan="4" className="px-6 py-8 text-center text-gray-500">No mentors found.</td></tr>
                    ) : mentors.map((m) => (
                      <tr key={m.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 flex justify-end gap-2">
                          <button onClick={() => setEditingMentor(m)} className="p-1.5 text-primary-600 hover:bg-primary-50 rounded-md transition-colors">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(m.id)} className="p-1.5 text-danger-600 hover:bg-danger-50 rounded-md transition-colors">
        ...
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h3 className="font-semibold text-lg">Edit Mentor</h3>
              <button onClick={() => setEditingMentor(null)} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleEditSubmit} className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
                <input required value={editingMentor.name} onChange={e => setEditingMentor({...editingMentor, name: e.target.value})} className="flex h-9 w-full rounded-md border border-gray-300 px-3 py-1 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="email" required value={editingMentor.email} onChange={e => setEditingMentor({...editingMentor, email: e.target.value})} className="flex h-9 w-full rounded-md border border-gray-300 px-3 py-1 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Department</label>
                <input required value={editingMentor.department} onChange={e => setEditingMentor({...editingMentor, department: e.target.value})} className="flex h-9 w-full rounded-md border border-gray-300 px-3 py-1 text-sm focus:ring-2 focus:ring-primary-500 outline-none" />
              </div>
              <div className="pt-2 flex justify-end gap-3">
                <Button type="button" variant="outline" onClick={() => setEditingMentor(null)}>Cancel</Button>
      ...
      {activeTab === 'single' && (
        <Card className="max-w-2xl">
          <CardHeader>
            ...
            <CardDescription>Enter the details to create a new mentor account.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit(onSingleSubmit)} className="space-y-4">
              ...
              <div className="pt-4 flex justify-end">
                <Button type="submit" disabled={singleLoading}>
                  {singleLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Saving...</> : 'Add Mentor'}
                </Button>
              </div>
            </form>
          </CardContent>
        ...
        <Card className="max-w-2xl">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription>Upload an Excel (.xlsx) file to add multiple mentors.</CardDescription>
            </div>
